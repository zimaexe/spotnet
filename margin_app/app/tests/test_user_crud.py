...
async def test_add_deposit_happy_path(user_crud: UserCRUD) -> None:
    ...
    """
    user = await user_crud.create_user(wallet_id = "wallet_123")
    deposit = await user_crud.add_deposit(user.id, amount=Decimal("100.00"))
...
async def test_add_deposit_non_existent_user(user_crud: UserCRUD) -> None:
    ...
    with pytest.raises(ValueError, match="User {non_existent_id} does not exist"):
        await user_crud.add_deposit(non_existent_id, amount=Decimal("100.00"))
...
async def test_add_deposit_invalid_amount(user_crud: UserCRUD) -> None:
    ...
    with pytest.raises(ValueError, match="Amount must be greater than zero"):
        await user_crud.add_deposit(user.id, amount=Decimal("0.00"))
#----------------------------------------------------------------------------------
...
async def test_add_margin_position_invalid_size(user_crud: UserCRUD) -> None:
    """
    ...
    with pytest.raises(ValueError, match="Leverage must be greater than zero"):
        await user_crud.add_margin_position(user.id, size=Decimal("0.00"), leverage=5)

async def test_add_margin_position_invalid_leverage(user_crud: UserCRUD) -> None:
    """
